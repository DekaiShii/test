...
class httpHandler(http.server.SimpleHTTPRequestHandler):
   ...
   def do_POST(self):
      ...
      #Rekognitionでじゃんけん
      if self.path == "/api/rek_janken/v1":
         rekognition = self.upload(form)
         message = self.janken(rekognition,1)

      if self.path == "/api/custom_label/v1":

         message = self.custom_label(form)

      if self.path == "/api/custom_label/v2":

         rekognition  = self.custom_label(form)
         message = self.janken(rekognition,2)
   ...
   def janken(self,message,flag):

           ji = 3

           if flag == 1:
                ji = self.jibun(message)

                if ji == -1:
                  return '{"jibun":-1, "aite":-1, "kekka":-1}'

           elif flag == 2:
                ji = self.custom_label_jibun(message)
           ...
           return_json = {"jibun":ji, "aite":ai, "kekka":kekka}

           return json.dumps(return_json, ensure_ascii=False)

   def custom_label(self,form):
      ...
      # exercise: AWS Rekognition extension
      import rekognition3
      data = rekognition3.detectLabels(UPLOADED_FILE)
      return json.dumps(data, ensure_ascii=False)
